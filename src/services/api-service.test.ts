...
import { PersistableCharacter } from '@models'
...
describe('ApiService', () => {
  ...
  describe('putCharacter', () => {
    test('returns a rejected promise if there is no token', () => {
      removeToken()
      return expect(ApiService.putCharacter('character-id', {} as PersistableCharacter)).rejects.toThrow(/token is missing/)
    })

    test('returns a rejected promise if the request fails', () => {
      fetchMock.mockRejectOnce(new Error('error'))
      return expect(ApiService.putCharacter('character-id', {} as PersistableCharacter)).rejects.toThrow(/error/)
    })
    ...
    test('returns the accepted response if the request succeeds', () => {
      ...
      return ApiService.putCharacter('character-id', {} as PersistableCharacter).then((data: any) => {
        expect(fetchMock).toHaveBeenCalledWith(
          'testing-url/character/character-id',
          {
            method: 'PUT',
